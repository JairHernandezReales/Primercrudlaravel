...
class ProductosController extends Controller
{
    ...
    public function update(Request $request, productos $productos)
    {
        ...
        $productos->precio_venta = $request->precio;
        $productos->save();
    ...
    public function inventario(Request $request)
    {
        ...
        $query = DB::table('inventario_resumen')
            ->when($busqueda, function ($query, $busqueda) {
                $query->where('codigo_producto', 'LIKE', '%'.$busqueda.'%')
                    ->orWhere('nombre_producto', 'LIKE', '%'.$busqueda.'%')
                    ->orWhere('nombre_categoria', 'LIKE', '%'.$busqueda.'%')
                    ->orWhere('existencia', 'LIKE', '%'.$busqueda.'%');
            })
            ->get();
